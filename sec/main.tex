...
This category is pointed, with zero object the zero complex~\(0\).
...
The \defn{Tate construction} is the functor \((-)^{tG} \colon \sD(R) \to \sD(\k)\) defined by
\[
M^{tG} \coloneqq \hocofib(\Nm \colon M_{hG} \to M^{hG}),
\]
and its homology groups are the \defn{Tate cohomology groups} \(\widehat{H}^*(G; M)\).
The resulting cofibre sequence
\[
M_{hG} \xrightarrow{\Nm} M^{hG} \longrightarrow M^{tG}
\]
in \(\sD(\k)\) relates group homology, group cohomology, and Tate cohomology when \(M\) is concentrated in degree 0.
