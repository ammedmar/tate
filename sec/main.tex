...
A result of Nikolaus and Scholze~\cite{??} shows that there is no chain map
\[
...
\[
[x] \longmapsto [\,e_{n-pn}\otimes_{\cyc_p} (x\otimes \cdots \otimes x)\,].
\]
We refer to $\Delta^t(p)$ as the \defn{algebraic Tate diagonal}.

\subsection{Monoidality}

Let $H \colon \sD(\k)\to \text{GrVect}_\k$ denote the homology functor. It is a lax monoidal functor: for chain complexes $C$ and $D$, the Künneth map defines a morphism
\[
...
\[
\sD(\k) \xra{(-)^{\otimes p}} \sD(R) \xra{(-)^{t\cyc_p}} \sD(\k) \xra{H} \text{GrVect}_\k.
\]
This is again a lax monoidal functor. Indeed, there is a morphism
\[
...
\subsection{cohomology operations} A hypothetic way of obtaining cohomology operations from the algebraic Tate diagonal is by following this diagram and passing to homology:

\input{sec/diagram_tate_diag_operations}

Another way is to notice that by postcomposing with the norm map, we can map 
\[
\Hom(C,(\widehat W\otimes C^{\otimes p})_{C_p})\to \Hom(C,(\widehat W\otimes C^{\otimes p})^{C_p})=\Hom_{C_p}(C,\widehat W\otimes C^{\otimes p}).
\]
Since $\widehat W$ is projective, we also have the isomorphism %i need to check this
\[
\Hom_{C_p}(C,\widehat W\otimes C^{\otimes p})\cong \Hom_{C_p}(C\otimes \widehat W^\vee, C^{\otimes p}).
\]
Thus, a tate-like diagonal map $C\to (C^\otimes p)^{tC_p}$ gives us a stable diagonal %or cup-$i$ construction?
\[
