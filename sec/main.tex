...
\[
(C^{\otimes p})^{t C_p}\cong \widehat W(p)\otimes_G C^{\otimes p}.
\]
...
\[
\Delta^t(p)\colon H_*(C)\to H_*((C^\otimes p)^{tC_p})=H_*(\widehat W\otimes_G C^{\otimes p})
\]
that sends a homology class $[x]$ in degree $n$ to $[e_{n-pn}\otimes_G (x\otimes \overset{_{(p)}}{\cdots}\otimes x)]$. We call $\Delta^t(p)$ the \defn{algebraic Tate diagonal}.

A hypothetic way of obtaining cohomology operations from the algebraic Tate diagonal is by following this diagram and passing to homology:

\input{sec/diagram_tate_diag_operations}

Another way is to notice that by postcomposing with the norm map, we can map 
\[
\Hom(C,(\widehat W\otimes C^{\otimes p})_G)\to \Hom(C,(\widehat W\otimes C^{\otimes p})^G)=\Hom_G(C,\widehat W\otimes C^{\otimes p}).
\]
Since $\widehat W$ is projective, we also have the isomorphism %i need to check this
\[
\Hom_G(C,\widehat W\otimes C^{\otimes p})\cong \Hom_G(C\otimes \widehat W^\vee, C^{\otimes p}).
\]
Thus, a tate-like diagonal map $C\to (C^\otimes p)^{tC_p}$ gives us a cup-$i$ construction 
\[
