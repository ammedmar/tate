...
%We write
%\[
%\widehat x_j \in \widehat H_j(\cyc_2;\Ftwo)
%\]
...
%\[
%\widehat x_p\ot e_\alpha\ot e_\alpha
%\]
...
%\[
%\bigoplus_\alpha \Ftwo[u,u^{-1}]\set{\widehat x_{-d_\alpha}\ot e_\alpha\ot e_\alpha}
%\xra{\cong} H(T_2V),
%\]
...
%	& v \arrow[r, |->] & \widehat{x}_{-|v|} \ot v \ot v.
%\end{tikzcd}
%\]
...
%	\[
%	\widehat x_{-d_\alpha}\ot e_\alpha\ot e_\alpha.
%	\]
%
%	By the defining property of the Tate diagonal,
%	\[
%	\Delta^t_*(e_\alpha)=\widehat x_{-d_\alpha}\ot e_\alpha\ot e_\alpha.
%	\]
%	Multiplication by \(u\) gives
%	\[
%	u^k\Delta^t_*(e_\alpha)=\widehat x_{k-d_\alpha}\ot e_\alpha\ot e_\alpha.
%	\]
...
%	Thus the formula
%	\[
%	v\mapsto \widehat x_{-|v|}\ot v\ot v
%	\]
...
%	\[
%	u^k\Delta^t_*(v)=\widehat x_{k-|v|}\ot v\ot v
%	\]
%	with
%	\[
%	k-|v|<0.
...
\section{Steenrod squares}\label{Steenrod} \martin{Rewrite this section to arbitrary p or write another section just for odd primes?}
Let $V$ be a chain complex in $\cD(\Ftwo)$.
...
%\[
%u \Delta^t_*(v) = u (\widehat x_{-|v|} \ot v \ot v) = \widehat x_{1-|v|} \ot v \ot v.
%\]
...
\subsubsection{Tate diagonal}
\begin{definition}
The \defn{algebraic Tate diagonal} is the natural transformation
\[
\Delta^t \colon \id_{\cD(\Ftwo)} \to T_2(-)
\]
whose existence promotes the non-linear assignment \(v \mapsto v \ot v\) to an actual map in \(\cD(\Ftwo)\), and which is characterized on homology by
