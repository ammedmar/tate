...
\section{Steenrod operations}\label{Steenrod}
...
\subsubsection{Another model for $T_p V$}

Let $\widetilde W_p\coloneqq \fib (W\overset{N}\longrightarrow W^\dd)$, the fibre of the norm map. Explicitly, $\widetilde W_p=\widehat W_p[-1]$ is the complex
\[
 \cdots \xla{-N} R\set{\tilde e_{-2}} \xla{\overline  T} R\set{\tilde e_{-1}} \xla{N} R\set{\tilde e_0} \xla{T} R\set{\tilde e_1} \xla{N} R\set{\tilde e_2} \xla{T} \cdots,
\]
with $\tilde e_i\coloneqq \hat e_{i+1}[-1]$ in degree $i$. Thus, $\widetilde W_p\otimes_{\cyc_p} M\simeq M^{t\cyc_p}[-1]$ for a $\Fp[\cyc_p]$-module $M$. However, if we take $M=\Fp$ with the trivial action, then 
\[
H(\widetilde W_p\otimes_{\cyc_p} \Fp)\cong \Fp\{\tilde e_i\}\cong \Fp[u,u^{-1}]\{\tilde e_0\},
\]
and we have an isomorphism 
	\[
	\begin{tikzcd}[row sep=0, column sep=small]
		H(\widetilde W_p\otimes_{\cyc_p} \Fp) \arrow[r] & H(\widehat W_p\otimes_{\cyc_p} \Fp) \\
		\hspace*{13pt} \tilde e_i \arrow[r, |->] & \hat e_i.
	\end{tikzcd}
	\]
This means that there are $\Fp[\cyc_p]$-modules for which both the fibre and the cofibre of the norm map give models for the tate construction. In virtue of Lemma \ref{pTate_iso},  this also happens for $V^{\ot p}$:
\begin{lemma} There are isomorphisms
\[
H(\widetilde W_p\otimes_{\cyc_p} V^{\ot p})\cong H(T_p V)\cong  H(\widehat W_p\otimes_{\cyc_p} V^{\ot p}).
\]
Consequently, $T_pV \simeq T_p V[-1]$ in $\cD(\Fp)$.
\end{lemma}
...
%\end{lemma}
%
\subsubsection{Tate multiplications and Steenrod operations}
