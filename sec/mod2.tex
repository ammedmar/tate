...
\end{tikzcd}
\]
The map \(\Delta^t_*\) is linear since the cross term \(v \ot w + w \ot v\) vanishes as it is a norm class.
...
%\begin{proof}
%	Both sides are free \(\Ftwo[u,u^{-1}]\)-modules and the map is \(\Ftwo[u,u^{-1}]\)-linear by construction.
%	It therefore suffices to check that it carries a basis of the source to a basis of the target.
%	By \cref{eq:tate_sequence} and the finite-type hypothesis, \(H(T_2 V)\) is a free \(\Ftwo[u,u^{-1}]\)-module on \(H(V)\), and the classes \(\Delta^t_*(v)\) for \(v\) ranging over a basis of \(H(V)\) form a generating set by the homology formula for \(\Delta^t_*\).
%	A free module of the correct rank generated by these classes is exactly the image of the displayed map, which is consequently an isomorphism.
%\end{proof}
...
%\begin{proof}
%	By definition \(\Sq_0^{\widehat\mu} = \widehat\mu_* \circ \Delta^t_* = (\widehat\mu \circ \Delta^t)_*\).
%	Over a field a morphism of \(\cD(\Ftwo)\) is determined by its effect on homology, so \(\Sq_0^{\widehat\mu} = \id_{H(V)}\) holds if and only if \((\widehat\mu \circ \Delta^t)_* = (\id_V)_*\), that is, if and only if \(\widehat\mu \circ \Delta^t \sim \id_V\).
%\end{proof}
...
%\begin{proof}
%	Applying \([-,V]\) to the distinguished triangle \cref{eq:tate_sequence} yields an exact sequence in which \(\widehat\mu\) factors through the projection \(T_2 V \to (V^{\ot 2})_{h\cyc_2}\) if and only if the composite \((V^{\ot 2})^{h\cyc_2}[-1] \to T_2 V \xra{\widehat\mu} V\) is null.
%	Over a field this composite is null if and only if \(\widehat\mu_*\) vanishes on the image of \(H\big((V^{\ot 2})^{h\cyc_2}[-1]\big)\) in \(H(T_2 V)\).
%	Under \cref{p:tate_iso} this image is spanned by the diagonal classes \(u^k \Delta^t_*(v) = \hat x_{k-|v|} \ot v \ot v\) with \(k - |v| < 0\), since the homotopy fixed point summand contributes exactly the negative powers of \(\hat x\).
%	Thus \(\widehat\mu_*\) kills it precisely when \(\Sq_k^{\widehat\mu}(v) = \widehat\mu_*\big(u^k \Delta^t_*(v)\big) = 0\) for all \(k < |v|\), which is the instability property.
%\end{proof}
...
%\begin{proof}
%	The equivalence \(T_2(V) \xra{\sim} T_2(\Ftwo) \ot V\) holds because both objects are formal of finite type with the homology computed in \cref{p:tate_iso}; we use it only to compare \(\widehat\mu\) against the linearized sequence, not as a claim that the quadratic functor \(T_2\) is linear.
%	As in the previous lemma, \(\widehat\mu\) factors through \(T_2(\Ftwo) \ot V \to (\Ftwo^{\ot 2})_{h\cyc_2} \ot V\) if and only if the composite from \((\Ftwo^{\ot 2})^{h\cyc_2}[-1] \ot V\) into \(V\) is null.
%	On homology the sub corresponds to the negative powers \(u^{-1}\Ftwo[u^{-1}] \ot H(V)\), that is, to the classes \(u^k \Delta^t_*(v)\) with \(k < 0\).
%	Hence \(\widehat\mu_*\) kills it precisely when \(\Sq_k^{\widehat\mu}(v) = 0\) for all \(k < 0\), which is the connectivity property.
%\end{proof}