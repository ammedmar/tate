...
\[
\Delta^t(p)(a[x])=[e_{(1-p)n}\ot_{C_p}(ax)^{\ot p}]=a^p[e_{(1-p)n}\ot_{C_p}x^{\ot p}]=a\Delta^t(p)([x]),
\]
...
%
%\begin{question}
%Does following this diagram and passing to homology give non trivial cohomology operations?
%
%ANSWER: No, since the norm map gives elements that are zero on homology, so the resulting composition is trivial.
%
