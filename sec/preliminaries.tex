...
\begin{definition}
	An associative and unital \defn{ring spectrum} is a \(\bE_1\)-algebra in \(\mathsf{Sp}\).
\end{definition}

For such a ring spectrum \(X\), its corresponding homotopy associative multiplication will be denoted by \(m_X^{\mathsf{Sp}}\). An important example will be studied in \cref{rcan}.
...
\begin{remark}\label{rcan}
	If \(A\) is a unital associative algebra over \(\Fp\) with multiplication \(m_A\), then, \(\sH A\) is a unital associative ring spectrum, since \(m_A\) induces via \(\sH\) a map \(m_{\sH A}^{\sH \Fp} \colon \sH A \ot_{\sH \Fp} \sH A \to \sH A\), and precomposition with the canonical projection gives \(m_{\sH A}^{\mathsf{Sp}}\).
	...
	\begin{center}
		\begin{tikzpicture}
			...
			\draw[->](2)--node[above]{\((m_{\sH \Fp}^{\sH \Fp})^{t\cyc_p}\)}(b);
			\draw[->](3)--node[above]{\((\sH m_{\Fp})^{t\cyc_p}\)}(c);
			\draw[->](4)--node[above]{\(\sH \left ((m_{\Fp})^{t\cyc_p}\right )\)}(d);
			%
